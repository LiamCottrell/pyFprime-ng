    ...
    if Orbs:
        for Orb in Orbs:
            ...
            if Orb["IfBe"] == 0 and BindEn >= KEv:
                ...
                Corr = (
                    0.5 * Orb["SEdge"] * BB**2 * math.log((RX - BB) / (-RX - BB)) / RX
                )
            else:
...
class PickElement(wx.Dialog):
    ...
    def _init_ctrls(self, prnt, list):
        ...
        for E in self.ElTable:
            PickElement.ElButton(
                self,
                parent=elPanel,
                name=E[0],
                pos=wx.Point(E[1] * 30 + 20, E[2] * 30 + 25),
                tip=E[3],
                color=E[4],
            )
            i += 1
...
class DeleteElement(wx.Dialog):
    ...
    def _init_ctrls(self, parent):
        ...
        wx.Dialog.__init__(
            ...
            parent=parent,
            pos=wx.DefaultPosition,
            size=wx.Size(max(128, 64 + l * 24), 87),
            style=wx.DEFAULT_DIALOG_STYLE,
