...
[
    wxID_FPRIMEKALPHAAGKA,
    wxID_FPRIMEKALPHACOKA,
    wxID_FPRIMEKALPHACRKA,
    wxID_FPRIMEKALPHACUKA,
    wxID_FPRIMEKALPHAFEKA,
    wxID_FPRIMEKALPHAMNKA,
    wxID_FPRIMEKALPHAMOKA,
    wxID_FPRIMEKALPHANIKA,
    wxID_FPRIMEKALPHAZNKA,
...
"""
    ...
    def _init_ctrls(self, parent):
        ...
        selSizer.Add((5, 10), 0)
        selSizer.Add(
            wx.StaticText(parent=panel, label="Wavelength:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.EXPAND,
        )
        selSizer.Add((5, 10), 0)
        self.SpinText1 = wx.TextCtrl(
            id=wxID_SPINTEXT1,
            parent=panel,
            size=wx.Size(100, 20),
        ...
        selSizer.Add(
            wx.StaticText(parent=panel, label="Energy:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.EXPAND,
        )
        selSizer.Add((5, 10), 0)
        self.SpinText2 = wx.TextCtrl(
            id=wxID_SPINTEXT2,
            parent=panel,
            size=wx.Size(100, 20),
        ...
        self.SpinButton.SetRange(int(10000.0 * self.Wmin), int(10000.0 * self.Wmax))
        self.SpinButton.SetValue(int(10000.0 * self.Wave))
        self.SpinButton.Bind(wx.EVT_SPIN, self.OnSpinButton, id=wxID_SPINBUTTON)

        self.slider1 = wx.Slider(
            id=wxID_FPRIMESLIDER1,
            maxValue=int(1000.0 * self.Wmax),
            minValue=int(1000.0 * self.Wmin),
            parent=panel,
            style=wx.SL_HORIZONTAL,
            value=int(self.Wave * 1000.0),
        )
        ...
        choiceSizer.Add(
            wx.StaticText(parent=panel, label="Plot scales:"),
            0,
            wx.ALIGN_CENTER_VERTICAL | wx.EXPAND,
        )
        choiceSizer.Add((5, 10), 0)

        self.choice1 = wx.ComboBox(
            id=wxID_FPRIMECHOICE1,
            parent=panel,
            value="Wavelength",
        ...
        self.choice2 = wx.ComboBox(
            id=wxID_FPRIMECHOICE2,
            value=" sin(" + Gktheta + ")/" + Gklambda,
    ...
    def OnSpinButton(self, event):
        if self.ifWave:
            Wave = float(self.SpinButton.GetValue()) / 10000.0
        else:
            Wave = self.Kev / (float(self.SpinButton.GetValue()) / 10000.0)
    ...
    def UpDateFPlot(self, Wave, rePlot=True):
        ...
        try:
            ...
            self.fplot = pylab.figure(
                facecolor="white", figsize=(8, 8)
            )  # BTW: default figsize is (8,6)
            self.fplot.canvas.set_window_title("X-Ray Resonant Scattering")
        ...
        if self.ifWave:
            bx.set_title(
                "%s%s%6.4f%s" % ("Form factors (", r"$\lambda=$", self.Wave, r"$\AA)$"),
                x=0,
                ha="left",
            )
        else:
        ...
        StlMax = min(2.0, math.sin(80.0 * math.pi / 180.0) / Wave)
        Stl = pylab.arange(0.0, StlMax, 0.01)
        Ymax = 0.0
        for i, Elem in enumerate(self.Elems):
            ...
            if Elem[1] > 78 and self.Energy > self.Kev / 0.16:
                res = 0.0
            if Elem[1] > 94 and self.Energy < self.Kev / 2.67:
                res = 0.0
            Els = Elem[0]
    ...
    def SetWaveEnergy(self, Wave):
        ...
        if self.ifWave:
            ...
            self.SpinButton.SetValue(int(10000.0 * self.Energy))
        Text = ""
        for Elem in self.Elems:
        ...
        if self.Elems:
            ...
            try:
                for Elem in self.Elems:
                    ...
                    if Z > 78:
                        Wmin = (
                            0.16  # heavy element high energy failure of Cromer-Liberman
                        )
                    if Z > 94:
                        Wmax = (
                            2.67  # heavy element low energy failure of Cromer-Liberman
                        )
                    lWmin = math.log(Wmin)
                    N = int(
    ...
    def OnChoice1(self, event):
        if event.GetString() == "Wavelength":
            ...
            self.slider1.SetValue(int(1000.0 * self.Energy))
            self.SpinButton.SetRange(int(10000.0 * Emin), int(10000.0 * Emax))
            self.SpinButton.SetValue(int(10000.0 * self.Energy))
            self.SpinText1.SetValue("%6.4f" % (self.Wave))
            self.SpinText2.SetValue("%7.4f" % (self.Energy))
        self.CalcFPPS()
